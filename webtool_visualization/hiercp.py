...
def make_hiercp(IMAGE_PATH, IMAGE_FILE, CP_PATH, EXTENSION):
    ...
    # to specify 
    # input: Image
    #IMAGE_PATH = r'D:\thesis\webtool_visualization\static\img' # load image 
    ...
    with h5py.File(f"palettes.hdf5", "w") as hdf5_file:
        ...
        for m, img_bgr in enumerate(images):   
            ...
            os.chdir(CP_PATH)
            # name it
            img_name = f"{IMAGE_FILE[m][:-4]}_hierlabcp.jpg"
    #            if not use_lab:
